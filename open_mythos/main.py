...
class MoEFFN(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # routed expert dispatch: sort tokens by expert so each expert is called
        # once on its full token batch, and we only pay one CPU-GPU sync total
        # (bincount) instead of one per expert per topk slot.
        out = torch.zeros_like(flat)
        N = flat.shape[0]
        token_ids_base = torch.arange(N, device=flat.device)

        all_expert_ids = topk_idx.reshape(-1)                                        # (N*topk,)
        all_scores     = topk_scores.reshape(-1)                                     # (N*topk,)
        all_token_ids  = token_ids_base.unsqueeze(1).expand(-1, self.topk).reshape(-1)

        perm           = all_expert_ids.argsort(stable=True)
        sorted_expert  = all_expert_ids[perm]
        sorted_tokens  = all_token_ids[perm]
        sorted_scores  = all_scores[perm]

        expert_counts = sorted_expert.bincount(minlength=self.n_experts).tolist()   # one sync
        offset = 0
        for eid, count in enumerate(expert_counts):
            if count:
                t_ids  = sorted_tokens[offset : offset + count]
                scores = sorted_scores[offset : offset + count].unsqueeze(-1)
                out.index_add_(0, t_ids, self.routed_experts[eid](flat[t_ids]) * scores)
            offset += count
...
class LoRAAdapter(nn.Module):
    ...
    def forward(self, x: torch.Tensor, loop_t: int) -> torch.Tensor:
        ...
        s = self.scale.weight[t_idx]  # (rank,) — direct index avoids tensor allocation
...
class RecurrentBlock(nn.Module):
    ...
    def __init__(self, cfg: MythosConfig):
        ...
        # Precomputed sinusoidal loop-index table: (max_loop_iters, dim).
        # Registered as a non-persistent buffer so it moves with the model but
        # is not saved in checkpoints (rebuilt from deterministic math on load).
        self.register_buffer(
            "_loop_emb",
            self._build_loop_emb(cfg.max_loop_iters, cfg.dim, cfg.dim // 8),
            persistent=False,
        )

    @staticmethod
    def _build_loop_emb(max_loops: int, dim: int, loop_dim: int, theta: float = 10000.0) -> torch.Tensor:
        freqs = 1.0 / (theta ** (torch.arange(0, loop_dim, 2).float() / loop_dim))
        t_idx = torch.arange(max_loops).float()
        angles = t_idx.unsqueeze(1) * freqs.unsqueeze(0)       # (max_loops, loop_dim//2)
        emb = torch.cat([angles.sin(), angles.cos()], dim=-1)[:, :loop_dim]  # (max_loops, loop_dim)
        emb_full = torch.zeros(max_loops, dim)
        emb_full[:, :loop_dim] = emb
        return emb_full
    ...
    def forward(
        ...
        for t in range(n_loops):
            # Clamp for depth extrapolation: inference may use more loops than
            # max_loop_iters; reuse the last table entry beyond the trained range.
            t_emb = min(t, self._loop_emb.shape[0] - 1)
            h_loop = h + self._loop_emb[t_emb].to(h.dtype)
...
class OpenMythos(nn.Module):
    ...
    def generate(
        ...
        # Pre-allocate the full output buffer to avoid growing allocations each step.
        out = torch.empty(B, prompt_len + max_new_tokens, dtype=input_ids.dtype, device=input_ids.device)
        out[:, :prompt_len] = input_ids

        kv_cache: dict = {}
        for step in range(max_new_tokens):
            if step == 0:
                cur_ids  = out[:, :prompt_len]
                start_pos = 0
            else:
                cur_ids  = out[:, prompt_len + step - 1 : prompt_len + step]
            ...
            out[:, prompt_len + step] = torch.multinomial(probs, num_samples=1).squeeze(-1)
        return out
